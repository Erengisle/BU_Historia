...
function warnBlock_(warnLevel, resultUrl) {
  ...
  if (warnLevel === 1) {
    ...
    warnBody  = 'För att klara kursen krävs minst 3 godkända prov. Du behöver höra av dig till mig så vi kan lägga upp en plan tillsammans.';
    ctaLabel  = 'Anmäl dig till omprov →';
    ctaHref   = getOmprovUrl();
    ctaBg = '#B52020'; ctaColor = '#fff';
    tipsHtml  = tips;
    ...
    warnBody  = 'Du har inte klarat tre eller fler prov. Det är nu mycket viktigt att du hör av dig till mig — vi måste prata om vad som krävs för att du ska ha en chans att klara kursen.';
...
function warnLevel_(results) {
  ...
  if (totalFail >= 3) return 3;
  if (totalFail >= 2) return 2;
  return 1;
}
...
function buildHtmlEmail(studentName, results, comments, resultUrl) {
  ...
  var tcColor = count >= 3 ? '#1A7A4A' : (count === 2 ? '#A07800' : '#B52020');
  var tcLabel = count >= 3 ? 'Du klarar kursen' : (count === 2 ? 'Nästan — ett prov kvar' : 'Fler godkända behövs');
  ...
  var counterHtml =
    '<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 16px;background:' + tcBg + ';border-radius:10px;">'
      + '<tr><td style="padding:14px 18px;">'
        + '<span style="font-family:Georgia,serif;font-size:32px;font-weight:700;color:' + tcColor + ';">' + count + '/5</span>'
      ...
      + '<tr><td style="background:#F7F4EE;padding:16px;border-radius:0 0 12px 12px;">'
        ...
        + '<tr><td style="padding:14px 16px;">'
          + '<p style="margin:0;font-family:Georgia,serif;font-style:italic;font-size:13px;color:#556070;line-height:1.75;">'
            + 'För godkänt på kursen krävs 3 av 5 godkända prov. Omprov planeras vid behov — alltid onsdagar kl. 14:45–16:15.'
          + '</p>'
        + '</td></tr></table>'
  ...
  + '<body>'
    ...
    + '<div class="content">'
      ...
      + '<div class="trafikljus">'
        + '<div class="trafikljus-siffra">' + count + '/5</div>'
